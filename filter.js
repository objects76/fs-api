...
var Util = {
  ...
  /*toDataURL: (contentType, uint8Array)=> {
...
FileError.prototype.__defineGetter__("name", ()=> {
...
var Filer = new (()=> {
   ...
   */
  var getEntry_ = (callback, var_args)=> {
   ...
   */
  var copyOrMove_ = (src, dest, opt_newName, opt_successCallback, opt_errorHandler, opt_deleteOrig)=> {
    if (!fs_) {
   ...
   */
  Filer.prototype.pathToFilesystemURL = (path)=> {
   ...
   */
  Filer.prototype.init = (opt_initObj, opt_successCallback, opt_errorHandler)=> {
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.ls = (dirEntryOrPath, successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }

    var callback = (dirEntry)=> {
      cwd_ = dirEntry;
    ...
    if (dirEntryOrPath.isDirectory) {
      // passed a DirectoryEntry.
      callback(dirEntryOrPath);
    } else if (isFsURL_(dirEntryOrPath)) {
      // passed a filesystem URL.
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.mkdir = (path, opt_exclusive, opt_successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.open = (entryOrPath, successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.create = (path, opt_exclusive, successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }
    ...
    cwd_.getFile(path, { create: true, exclusive: exclusive }, successCallback, (e)=> {
      if (e.code == FileError.INVALID_MODIFICATION_ERR) {
        e.message = "'" + path + "' already exists";
      }
      if (opt_errorHandler) {
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.mv = (src, dest, opt_newName, opt_successCallback, opt_errorHandler)=> {
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.rm = (entryOrPath, successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.cd = (dirEntryOrPath, opt_successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.cp = (src, dest, opt_newName, opt_successCallback, opt_errorHandler)=> {
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.write = (entryOrPath, dataObj, opt_successCallback, opt_errorHandler)=> {
    if (!fs_) {
      throw new Error(FS_INIT_ERROR_MSG);
    }

    var writeFile_ = (fileEntry)=> {
    ...
    if (entryOrPath.isFile) {
      ...
      cwd_.getFile(entryOrPath, { create: true, exclusive: false }, writeFile_, opt_errorHandler);
    }
  };
   ...
   * @param {Function=} opt_errorHandler Optional error callback.
   */
  Filer.prototype.df = (successCallback, opt_errorHandler)=> {
    var queryCallback = (byteUsed, byteCap)=> {
